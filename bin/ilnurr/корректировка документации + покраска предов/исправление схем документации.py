...
for osn1 in range(len(AreaID)):
                ...
                for var1_1 in variantID:
                    ElementID1 = cursorObj.execute(
                        "SELECT Element_ID, Variant_ID FROM Element WHERE Name IN ({0}) AND Variant_ID IN ({1})".format(
                            str("'" + NameEl + "'"), str(var1_1))).fetchall()
                    if ElementID1 != []:
                        break
    ...
    for osn3 in range(len(NodeID)):
        NameNd = cursorObj.execute("SELECT Name FROM Node WHERE Node_ID IN ({0})".format(
            str(NodeID[osn3]))).fetchone()[0]
        NodeID1 = []
        for var2 in variantID:
            NodeID1 = cursorObj.execute(
                "SELECT Node_ID, Variant_ID FROM Node WHERE Node_ID NOT LIKE ({0}) AND Name IN ({1}) AND Variant_ID IN ({2})".format(
                    str(NodeID[osn3]), str("'" + NameNd + "'"), str(var2))).fetchall()
            if NodeID1 != []:
                break
    ...
    for osn4 in range(len(TerminalID)):
        ...
        print('ТЕРМИНАЛЫ1', GTerminalID1, TerminalID1)
        ...
        print('ПОИНТ АЙДИ  ', PointID, GTerminalID1[2])
