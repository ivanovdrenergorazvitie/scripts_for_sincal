...
for i in range(len(typID)):
    if i == 0 or i == 32:
        continue
    tr_for_db += [cursor.execute(f"select Element_ID, TwotTyp from StdTwoWindingTransformer where Element_ID in ({typID[i]})").fetchone()]
...
for var in variantID:
    transID0 = cursorObj.execute(f"SELECT Element_ID, Typ_ID, Un1, Sn FROM TwoWindingTransformer WHERE Variant_ID IN ({var})").fetchall()
...
i = 0
while i < len(transID):
    transtrans = cursorObj.execute(f"SELECT Element_ID FROM Element WHERE Name IN ('{transID[i][4]}')").fetchall()
...
error_el = []
for i in range(len(transID)):
    voltlevelID = cursorObj.execute(f"SELECT Element_ID, VoltLevel_ID FROM Element WHERE Element_ID IN ({transID[i][0]})").fetchone()
